...
def check_placements(placements: Placements) -> None:
    ...
    if num_sharded == 0:
        raise ValueError(
            "a fully replicated tensor is carried as REPLICATED, not as two "
            "REPLICATE axes"
        )
    if num_sharded == MESH_NDIMS:
        raise ValueError(
            f"nccl_m2n needs one REPLICATE mesh axis, but {placements} shards both"
        )


def resolve_layout(
    mesh: M2NMesh, placements: Placements | None
